...
(implicit entityReader: BSONDocumentReader[E], entityWriter: BSONDocumentWriter[E],
 ...
 idReader: BSONReader[_ <: BSONValue, K]) extends RadioactiveMongoTemplate[E,K] {
  ...
  def findAll(query: BSONDocument = BSONDocument.empty,sort: BSONDocument = BSONDocument("_id" -> 1)):
  Future[List[E]] = {
